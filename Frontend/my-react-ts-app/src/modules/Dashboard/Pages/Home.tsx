  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="from-[#0b3b68] to-[#1f4e7a] text-white rounded-xl p-8 shadow-lg">
        <h1 className="text-3xl font-bold mb-2">¡Bienvenido a Ferretería Central! 🛠️</h1>
        ...
        <div className="mt-6 flex flex-wrap gap-4">
          <button className="bg-white text-[#0b3b68] px-5 py-2 rounded-lg font-semibold hover:bg-gray-200 transition">
            Ver Inventario
          </button>
          <button className="bg-[#ffcc00] text-[#0b3b68] px-5 py-2 rounded-lg font-semibold hover:bg-yellow-400 transition">
            Registrar Venta
          </button>
        </div>
      </div>
      ...
      <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: "Consultar Inventario", icon: <FaClipboardList size={24} />, color: "bg-blue-100 text-blue-800" },
          { label: "Registrar Producto", icon: <FaBoxOpen size={24} />, color: "bg-green-100 text-green-800" },
          { label: "Ver Reportes", icon: <FaShoppingCart size={24} />, color: "bg-yellow-100 text-yellow-800" },
        ].map((action, i) => (
          <motion.div
            key={i}
